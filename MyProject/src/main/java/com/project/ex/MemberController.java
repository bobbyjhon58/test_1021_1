...
public class MemberController {
	...
	@RequestMapping("/login")  // get 방식 호출시
	public String login() {
		return "login";
	}
	
	@RequestMapping(value="/login", method=RequestMethod.POST)    //post 방식일 때 호추
	@ResponseBody
	public Map<String, Object> login(String m_id, String m_pw){
		
		Map<String, Object> map = new Hashtable<String, Object>();
		
		// 인자 두개를 MemDAO의 login함수를 호출하면서 전달하면 정확한 정보일 대만
		// MemVO 한개를 받게 된다. 
		MemVO mvo = m_dao.login(m_id, m_pw);
		
		// mvo가  null 이면 아이디 및 비밀번호를 잘 못 입력한 경우 그렇지 않으면 잘 입력한 경우
		if(mvo != null) {
